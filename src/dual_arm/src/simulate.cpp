...
#include "../../../../mujoco200_linux/include/mjxmacro.h"
#include "../../../../mujoco200_linux/include/uitools.h"
#include "stdio.h"
#include "string.h"
...
//controller
CController Control(JDOF);

//ROS
void JoyCallback(const std_msgs::Float64MultiArray::ConstPtr& joy_msgs);
...
{
    int i = 0;
    for(std::vector<double>::const_iterator it = joy_msgs->data.begin(); it != joy_msgs->data.end(); ++it)
	{
		_joy_command[i] = *it;
		i++;
        //cout << "joy command " << i << endl << _joy_command[i] << endl;
	}
    return;
}
