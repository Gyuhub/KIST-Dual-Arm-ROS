...
void CModel::Initialize()
{
	...
	_bool_position_orientation_update = false; // !!!!!!!!!!!!!!!!!!!!!
	...
	_q_right_hand.w() = 0.0;
	_q_vec_left_hand.setZero();
	_q_vec_right_hand.setZero();
...
void CModel::calculate_EE_positions_orientations()
{
	if (_bool_kinematics_update == true)
	{
		...
		_bool_position_orientation_update = true; // !!!!!!!!!!!!!!!!!!!!!
	}
	else
	{
		cout << "Robot kinematics is not ready. Please update kinematics first." << endl << endl;
...
void CModel::calculate_Euler_To_Quat() // !!!!!!!!!!!!!!!!!!!!!
{
	if (_bool_position_orientation_update == true)
	{
		...
		_q_scalar_left_hand = _q_left_hand.w();
		_q_scalar_right_hand = _q_right_hand.w();
	}
	else
	{
